...
fn print_usage() {
    println!(
        "usage: fetch_user_activity --user <wallet> [--type <value>] [--market <slug>] [--event-id <value>] [--start <ts>] [--end <ts>] [--side <BUY|SELL>] [--limit <n>] [--offset <n>] [--sort-by <field>] [--sort-direction <ASC|DESC>] [--curl-bin <path>] [--print-url] [--print-curl]"
    );
}
...
#[cfg(test)]
mod tests {
    use super::{build_curl_args, build_url, parse_args};
