...
fn render_operator_demo_report(root: &Path) -> Result<String, RootEnvLoadError> {
    ...
    lines.push(
        "watch_copy_leader_activity_hint=cd rust-copytrader && cargo run --bin watch_copy_leader_activity -- --root .. --proxy http://127.0.0.1:7897 --poll-count 1"
            .to_string(),
    );
    lines.push(
    ...
    lines.push(
        "run_copytrader_auto_guarded_loop_hint=cd rust-copytrader && cargo run --bin run_copytrader_auto_guarded_loop -- --root .. --proxy http://127.0.0.1:7897 --watch-poll-count 1 --loop-count 1 --live-submit-gate"
            .to_string(),
    );
    lines.push(
        "run_copytrader_live_submit_gate_hint=cd rust-copytrader && cargo run --bin run_copytrader_live_submit_gate -- --root .. --activity-source-verified --activity-under-budget --activity-capability-detected --positions-under-budget"
            .to_string(),
    );
    lines.push(
        "run_position_targeting_demo_hint=cd rust-copytrader && cargo run --bin run_position_targeting_demo -- --root .."
            .to_string(),
    );
    lines.push(
        "run_copytrader_ansi_dashboard_hint=cd rust-copytrader && cargo run --bin run_copytrader_ansi_dashboard -- --root .."
            .to_string(),
    );
    lines.push(
    ...
    lines.push(
        "run_rust_minmax_follow_live_script_hint=bash scripts/run_rust_minmax_follow_live.sh --user <wallet>"
            .to_string(),
    );
    lines.push(
...
mod tests {
    ...
    fn operator_demo_report_includes_discovery_hints() {
        ...
        assert!(report.contains("run_copytrader_auto_guarded_loop_hint=cd rust-copytrader && cargo run --bin run_copytrader_auto_guarded_loop -- --root .. --proxy http://127.0.0.1:7897 --watch-poll-count 1 --loop-count 1 --live-submit-gate"));
        ...
        assert!(
            report.contains("run_rust_minmax_follow_live_script_hint=bash scripts/run_rust_minmax_follow_live.sh --user <wallet>")
        );
