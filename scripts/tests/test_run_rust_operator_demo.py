...
class RunRustOperatorDemoScriptTest(unittest.TestCase):
    def write_stub_cargo(self, root: Path) -> Path:
        ...
        stub_path.write_text(
            textwrap.dedent(
                ...
                sys.stdout.write("discover_copy_leader_hint=cd rust-copytrader && cargo run --bin discover_copy_leader -- --discovery-dir ../.omx/discovery --category SPECIALIST\\n")
                sys.stdout.write("smart_wallet_filter_hint=discover_copy_leader 严格按 rust-copytrader/wallet_filter_v1.md 做 smart money 筛选\\n")
                sys.stdout.write("scan_copy_leader_categories_hint=cd rust-copytrader && cargo run --bin scan_copy_leader_categories -- --discovery-dir ../.omx/discovery --categories SPECIALIST --proxy http://127.0.0.1:7897 --limit 1\\n")
                sys.stdout.write("run_copytrader_operator_flow_hint=cd rust-copytrader && cargo run --bin run_copytrader_operator_flow -- --root .. --discovery-dir ../.omx/discovery --live-submit-gate\\n")
                ...
                sys.stdout.write("run_copytrader_ansi_dashboard_hint=cd rust-copytrader && cargo run --bin run_copytrader_ansi_dashboard -- --root ..\\n")
    ...
    def test_run_rust_operator_demo_executes_cargo_operator_demo(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("smart_wallet_filter_hint=", result.stdout)
            self.assertIn("scan_copy_leader_categories_hint=", result.stdout)
            ...
            self.assertIn("run_copytrader_ansi_dashboard_hint=", result.stdout)
